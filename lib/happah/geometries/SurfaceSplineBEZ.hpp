...
std::tuple< TriangleMesh<Vertex>, std::vector<hpcolor> > make_colored_diamonds_mesh(const SurfaceSplineBEZ<Space, degree>& surface, hpuint nSubdivisions, hpcolor diamonds_color, hpcolor bg_color, VertexFactory&& factory = VertexFactory() );
...
template<class Space, hpuint degree, class Vertex, class VertexFactory, typename>
std::tuple< TriangleMesh<Vertex>, std::vector<hpcolor> > make_colored_diamonds_mesh(const SurfaceSplineBEZ<Space, degree>& surface, hpuint nSubdivisions, hpcolor diamonds_color, hpcolor bg_color, VertexFactory&& factory) {
     
     using Point = typename Space::POINT;
     static_assert(degree > 0u, "Constant spline surfaces cannot be converted into triangle meshes.");
     static_assert(std::is_base_of<Vertex, decltype(factory(Point(0.0)))>::value, "The vertex generated by the factory must be a subclass of the vertex with which the triangle mesh is parameterized.");

     auto do_make_colored_diamonds_mesh = [&](const SurfaceSplineBEZ<Space, degree>& surface) -> auto {
          auto vertices = std::vector<Vertex>();
          vertices.reserve(surface.getControlPoints().size());
          for(auto& point : surface.getControlPoints()) vertices.push_back(factory(point));
          auto indices = Indices();
          std::vector<hpcolor> colors;
          int dataSize = 3 * make_control_polygon_size(degree) * size(surface);
          indices.reserve(dataSize);
          colors.reserve(dataSize);
          auto triangleInserter = make_back_inserter(indices);
          
          visit_patches<degree>(std::begin(std::get<1>(surface.getPatches())), size(surface), [&](auto patch) {
               visit_deltas(degree, patch, triangleInserter);
               visit_nablas(degree, patch, triangleInserter);

               hpuint rowCounter = 0;
               hpuint remaining = degree;
               hpuint numberDeltas = ( degree * (degree + 1) ) / 2;
               for(hpuint j = 0; j < numberDeltas; j++){
                    if(rowCounter == 0){ //first row, color every triangle, no skip
                         colors.insert(colors.end(), 3, diamonds_color);
                    }else if(rowCounter < degree - 1) { //other rows, color first and last
                         if( (remaining == 1) || (remaining == degree - rowCounter) ){
                              colors.insert(colors.end(), 3, diamonds_color);
                         }else{
                              colors.insert(colors.end(), 3, bg_color);
                         }
                    }else{ //last row, one triangle to color, then done
                         colors.insert(colors.end(), 3, diamonds_color);
                    }
                    remaining--;
                    if(remaining == 0){
                         rowCounter++;
                         remaining = degree - rowCounter;
                    }
               }
               hpuint numberNablas = ( (degree-1) * degree) / 2;
               for(hpuint i = 0; i < numberNablas; i++){
                    colors.insert(colors.end(), 3, bg_color);
               }
          });
          
          return std::make_tuple(make_triangle_mesh(std::move(vertices), std::move(indices)), colors);
     };

     if(nSubdivisions > 0) return do_make_colored_diamonds_mesh(subdivide(surface, nSubdivisions));
     else return do_make_colored_diamonds_mesh(surface);
}
