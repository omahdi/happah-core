...
#include "happah/utils/ShortestPathFinder.hpp"
     ...
     static void sort(const Mesh& mesh, Iterator begin, Iterator end) {
          ...
          while(true) {
               while(!rests.empty() && rests.back().size() > 0) {
                    ...
                    visit_ring(mesh.getEdges(), mesh.getNumberOfTriangles(), mesh.getOutgoing(paths.back().back()), [&](hpuint v) { neighbors.push_back(v); });
