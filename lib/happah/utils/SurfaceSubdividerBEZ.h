...
public:
     ...
     std::pair<ControlPoints, Indices> subdivide(hpuint nSubdivisions) {
          ...
          hpuint nRows = 1 << nSubdivisions;//NOTE: A row consists of zero or more rectangles followed by a triangle.
          hpuint nRectangles = (nRows * (nRows - 1)) >> 1;//NOTE: The number of rectangles is the result of the arithmetic series for (nRows-1).  The first row has no rectangles and each following row has one rectangle more than the previous row.
          hpuint nTriangles = (nRectangles << 1) + nRows;
          hpuint nEdges = 3 * (nRectangles + nRows);//NOTE: We count the three bottom left edges in each triangle so as not to count the top edge and the right edge are not counted twice.  Then, three edges per triangle at the end of each row remain to be counted.
          hpuint nFacePoints = nTriangles * FACE_STRIDE;//NOTE: This is the number of points not on the border.
          hpuint nEdgePoints = nEdges * EDGE_STRIDE;
          hpuint nVertexPoints = nRectangles + (nRows << 1) + 1;

          ControlPoints facePointsA;
          ControlPoints facePointsB;
          ControlPoints facePointsC;//stores two temporary faces generated by first binary subdivision
          facePointsA.resize(nFacePoints);
          facePointsB.resize(nFacePoints);
          facePointsC.resize(FACE_STRIDE << 1);
          ControlPoints edgePointsA;
          ControlPoints edgePointsB;
          edgePointsA.resize(nEdgePoints);
          edgePointsB.resize(nEdgePoints);
          ControlPoints vertexPointsA;
          ControlPoints vertexPointsB;
          vertexPointsA.resize(nVertexPoints);
          vertexPointsB.resize(nVertexPoints);
          ...
          ControlPoints* sFacePoints = &facePointsA;//source points
          ControlPoints* sEdgePoints = &edgePointsA;
          ControlPoints* sVertexPoints = &vertexPointsA;
          ControlPoints* tFacePoints = &facePointsB;//target points
          ControlPoints* tEdgePoints = &edgePointsB;
          ControlPoints* tVertexPoints = &vertexPointsB;
          for(hpuint i = 1; i < nSubdivisions; ++i) {
               hpuint snRows = 1 << i;
               hpuint snNonHorizontalEdges = snRows * (snRows + 1);
               hpuint tnRows = 1 << (i + 1);
               hpuint tnNonHorizontalEdges = tnRows * (tnRows + 1);
          ...
          hpuint vt = nFacePoints + nEdgePoints;
          hpuint vb = vt + 1;
          hpuint ev = nFacePoints;
          hpuint ehb = ev + nRows * (nRows + 1) * EDGE_STRIDE;//top horizontal edge
          hpuint eht = ehb;//bottom horizontal edge
          hpuint f = 0;
          hpuint limit, nMiddlePoints;
