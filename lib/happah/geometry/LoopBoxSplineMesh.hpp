...
LoopBoxSplineMesh<Vertex> make_loop_box_spline_mesh(const TriangleMesh<Vertex>& mesh) {
     ...
     visit_triplets(mesh.getIndices(), [&](auto i0, auto i1, auto i2) {
          if(valences[i0] == 6 && valences[i1] == 6 && valences[i2] == 6) {//TODO: and not on border
               auto ring0 = make(make_ring_enumerator(neighbors, t, hptrit(0), [&](auto t, auto i) { return mesh.getIndices()[3 * t + i]; }));
               auto ring1 = make(make_ring_enumerator(neighbors, t, hptrit(1), [&](auto t, auto i) { return mesh.getIndices()[3 * t + i]; }));
               auto ring2 = make(make_ring_enumerator(neighbors, t, hptrit(2), [&](auto t, auto i) { return mesh.getIndices()[3 * t + i]; }));
